...
def plot_operation_comparison(df, output_dir):
    ...
    grouped = df_ops.groupby(['tool', 'operation', 'file_size_kb']).agg({
    ...
    for file_size in grouped['file_size_kb'].unique():
        ...
        plt.bar(x_pos - width/2, edlicense_heights, width, yerr=edlicense_errors,
                label='edlicense', color='#5d9cf5', capsize=5)
        plt.bar(x_pos + width/2, addlicense_heights, width, yerr=addlicense_errors,
                label='addlicense', color='#f55d5d', capsize=5)
        
        # Add labels, title and legend
        plt.xlabel('Operation')
        plt.ylabel('Duration (ms)')
        plt.title(f'Performance Comparison - {file_size}KB Files')
...
def plot_thread_impact(df, output_dir):
    ...
    grouped = df_threads.groupby('thread_count').agg({
        'duration_ms': ['mean', 'std']
    }).reset_index()
    ...
    plt.ylabel('Duration (ms)')
    plt.title('Impact of Thread Count on edlicense Performance')
    plt.grid(axis='y', linestyle='--', alpha=0.7)
...
def plot_file_size_impact(df, output_dir):
    ...
    # Group by relevant factors and calculate mean duration
    grouped = df_sizes.groupby(['tool', 'operation', 'file_size_kb']).agg({
        'duration_ms': ['mean']
    }).reset_index()
    ...
    for operation in grouped['operation'].unique():
        ...
        for tool in tools:
            ...
            plt.plot(tool_data['file_size_kb'], tool_data['duration_ms_mean'], 
...
def generate_summary_table(df, output_dir):
    ...
    grouped = df_summary.groupby(['tool', 'operation', 'file_size_kb']).agg({
    ...
    for op in grouped['operation'].unique():
        for size in sorted(grouped['file_size_kb'].unique()):
            ed_data = grouped[(grouped['tool'] == 'edlicense') & 
                             (grouped['operation'] == op) & 
                             (grouped['file_size_kb'] == size)]
            
            add_data = grouped[(grouped['tool'] == 'addlicense') & 
                             (grouped['operation'] == op) & 
                             (grouped['file_size_kb'] == size)]
...
def plot_speedup_comparison(summary_df, output_dir):
    ...
    for i, operation in enumerate(operations):
        ...
        plt.bar(x_pos + (i - 1) * width, ratios, width, 
                label=operation, alpha=0.8)
    ...
    plt.xticks(x_pos, [str(size) for size in file_sizes])
    plt.legend()
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    ...
    <body>
        ...
        <div class="summary">
            ...
            <p>
                ...
                across different operations and file sizes.
            </p>
        </div>
...
def main():
    ...
    parser.add_argument("--results-dir", default="target/benchmark_results",
                        help="Directory containing benchmark JSON results")
    parser.add_argument("--output-dir", default="benchmark_visualizations",
