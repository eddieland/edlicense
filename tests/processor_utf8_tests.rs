...
#[tokio::test]
async fn test_invalid_utf8_at_start_errors_gracefully() -> Result<()> {
  // When invalid UTF-8 is at the very start of a file (valid_up_to == 0),
  // the processor should return an error rather than panic.
  let temp_dir = tempdir()?;

  let processor = create_test_processor(&temp_dir, "Copyright (c) {{year}} Test Company", false, false).await?;
  ...
  // Process should error gracefully since invalid UTF-8 is at position 0
  let result = processor.process_file(&file_path).await;
  assert!(result.is_err(), "Expected error when file starts with invalid UTF-8");

  // Verify error message mentions UTF-8 or file reading issue
  let err_msg = result.unwrap_err().to_string();
  assert!(
    err_msg.contains("UTF") || err_msg.contains("read") || err_msg.contains("invalid"),
    "Error message should indicate UTF-8 or read failure: {}",
    err_msg
  );
...
#[tokio::test]
async fn test_invalid_utf8_after_valid_prefix() -> Result<()> {
  // When there's valid UTF-8 before invalid bytes, the processor should
  // read the valid portion and attempt to process it.
  let temp_dir = tempdir()?;

  let _processor = create_test_processor(&temp_dir, "Copyright (c) {{year}} Test Company", false, false).await?;
  ...
  let check_processor = create_test_processor(&temp_dir, "Copyright (c) {{year}} Test Company", true, false).await?;

  // Check-only should succeed since we can read the license prefix
  let result = check_processor.process_file(&file_path).await;
  assert!(
    result.is_ok(),
...
#[tokio::test]
async fn test_invalid_utf8_in_copyright_year() -> Result<()> {
  // Test that year detection handles corrupted year gracefully.
  // A copyright line like "Copyright (c) 20\xff24" shouldn't panic.
  let temp_dir = tempdir()?;

  let processor = create_test_processor(&temp_dir, "Copyright (c) {{year}} Test Company", true, false).await?;
...
#[tokio::test]
async fn test_invalid_utf8_after_copyright_symbol() -> Result<()> {
  // Test handling of invalid UTF-8 immediately after the © symbol
  let temp_dir = tempdir()?;

  let processor = create_test_processor(&temp_dir, "Copyright © {{year}} Test Company", true, false).await?;
...
#[tokio::test]
async fn test_invalid_utf8_after_shebang() -> Result<()> {
...
#[tokio::test]
async fn test_valid_shebang_with_invalid_body() -> Result<()> {
...
#[tokio::test]
async fn test_elf_binary_file() -> Result<()> {
...
#[tokio::test]
async fn test_png_image_file() -> Result<()> {
  ...
  // Should error since PNG magic bytes aren't valid UTF-8
  let result = processor.process_file(&file_path).await;
  assert!(result.is_err(), "PNG binary should error or be skipped");

  Ok(())
}

#[tokio::test]
async fn test_pdf_file() -> Result<()> {
...
#[tokio::test]
async fn test_valid_license_header_with_invalid_body() -> Result<()> {
...
#[tokio::test]
async fn test_modify_mode_with_invalid_utf8_body() -> Result<()> {
  // In modify mode, if we need to read the full file and it contains invalid
  // UTF-8, we should error rather than corrupt the file.
  let temp_dir = tempdir()?;
  ...
  // Modify mode needs to read full content to add license - should error on
  // invalid UTF-8
  let result = processor.process_file(&file_path).await;
  assert!(
    result.is_err(),
    "Modify mode should error when full file has invalid UTF-8"
  );
...
#[tokio::test]
async fn test_scattered_invalid_bytes() -> Result<()> {
...
#[tokio::test]
async fn test_incomplete_multibyte_sequence() -> Result<()> {
...
#[tokio::test]
async fn test_overlong_utf8_encoding() -> Result<()> {
...
#[tokio::test]
async fn test_null_bytes_in_file() -> Result<()> {
...
#[tokio::test]
async fn test_utf16_bom() -> Result<()> {
  ...
  // Should error since UTF-16 is not valid UTF-8
  let result = processor.process_file(&file_path).await;
  assert!(result.is_err(), "UTF-16 content should error: {:?}", result);

  Ok(())
}

#[tokio::test]
async fn test_utf8_bom_handling() -> Result<()> {
...
#[tokio::test]
async fn test_replacement_character_in_source() -> Result<()> {
