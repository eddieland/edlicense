...
fn generate_test_files(
    ...
    println!("Generating {} test files across {} subdirectories...", count, subdirs_count);
...
fn run_benchmark<F>(name: &str, iterations: usize, setup_fn: impl Fn() -> Result<F>, test_fn: impl Fn(&F) -> Result<()>) -> Result<()> {
