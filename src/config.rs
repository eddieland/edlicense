...
pub fn discover_config_path(explicit_path: Option<&Path>, workspace_root: &Path) -> Option<PathBuf> {
  ...
  if let Some(path) = explicit_path {
    if path.exists() {
      info!("Using explicit config path: {}", path.display());
      return Some(path.to_path_buf());
    }
    info!("Explicit config path does not exist: {}", path.display());
    return None;
  }
  ...
  if let Ok(env_path) = std::env::var(CONFIG_ENV_VAR) {
    ...
    if path.exists() {
      info!("Using config from {}: {}", CONFIG_ENV_VAR, path.display());
      return Some(path);
    }
    info!("{} path does not exist: {}", CONFIG_ENV_VAR, env_path);
  }

  // 3. Check workspace root
  let workspace_config = workspace_root.join(DEFAULT_CONFIG_FILENAME);
  if workspace_config.exists() {
    info!("Using workspace config: {}", workspace_config.display());
